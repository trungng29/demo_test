...
export async function getFlights(
  ...
  flightResults = await Promise.all(
    flightResults.map(async (flight) => {
      if (!Array.isArray(flight.segmentIds) || flight.segmentIds.length === 0) {
        return null;
      }
      ...
      const airplaneModelName = flight.segmentIds?.[0]?.airplaneId?.model;
      const flightReviews = await getManyDocs(
        "FlightReview",
        {
          ...
          ...(airplaneModelName && { airplaneModelName }),
        },
        ["flightReviews"],
      );
