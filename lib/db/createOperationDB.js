...
export async function createManyDocs(modelName, dataArr, options = {}) {
  ...
  try {
    const batchSize = Math.max(1, Number.parseInt(options.batchSize, 10) || 1000);
    const bulkOptions = { ...options };
    delete bulkOptions.batchSize;
    ...
    for (let i = 0; i < dataArr.length; i += batchSize) {
      ...
      insertedIds = insertedIds.concat(
        Object.values(returened.insertedIds || {}).map((id) => id.toString()),
      );
    }
